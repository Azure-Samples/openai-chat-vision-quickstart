class SpeechInputButton extends HTMLElement {
    ...
    startRecording() {
        if (this.speechRecognition == null) {
            this.dispatchEvent(new CustomEvent('speech-input-error', { detail: { error: "SpeechRecognition not supported" } }));
        }
        ...
        this.speechRecognition.onresult = (event) => {
            ...
            this.dispatchEvent(new CustomEvent('speech-input-result', { detail: { transcript: input } }));
        };
        ...
        this.speechRecognition.onerror = (event) => {
            if (this.speechRecognition) {
                ...
                if (event.error == "no-speech") {
                    this.dispatchEvent(new CustomEvent('speech-input-error', { detail: { error: "No speech was detected. Please check your system audio settings and try again." } }));
                } else if (event.error == "language-not-supported") {
                    this.dispatchEvent(new CustomEvent('speech-input-error', { detail: { error: "The selected language is not supported. Please try a different language." } }));
                } else {
                    this.dispatchEvent(new CustomEvent('speech-input-error', { detail: { error: "An error occurred while recording. Please try again." } }));
                }
            }
        };
