class SpeechOutputButton extends HTMLElement {
    ...
    constructor() {
        ...
        if (!SpeechSynthesis) {
            this.dispatchEvent(new CustomEvent('speech-output-error', { detail: { error: "SpeechSynthesis not supported" } }));
            return;
        }
    ...
    connectedCallback() {
        ...
        this.speechButton.addEventListener('click', () => this.toggleSpeechOutput());
    }
    ...
    toggleSpeechOutput() {
        ...
        if (this.synth != null) {
            ...
            let voice = this.synth.getVoices().filter((voice) => voice.lang === this.lngCode)[0];
            if (!voice) {
                voice = this.synth.getVoices().filter((voice) => voice.lang === "en-US")[0];
            }
